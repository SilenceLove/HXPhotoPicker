...
- (void)photoViewChangeComplete:(NSArray<HXPhotoModel *> *)allList Photos:(NSArray<HXPhotoModel *> *)photos Videos:(NSArray<HXPhotoModel *> *)videos Original:(BOOL)isOriginal
{
    ...
    // 为了优化相册列表以及预览大图列表快速滑动内存暴增的问题，如果缓存了imageData或者原图的image 如果用户图片过多时这会导致内存的增大,而且当快速滑动遇到图片过大时可能导致滑动卡顿 / 内存警告⚠️程序被杀。故不缓存imageData和image 只保留 thumbPhoto 缩略图   这样可以保证在选择照片/快速滑动过程中,不会因为内存过大导致程序被杀 和 滑动流畅丝滑  。所以要获取已选图片的原图可以选择本人提供的快速获取已选照片的全部原图，也可以自己根据指定方法获取不同质量的图片。提醒：在用户没有选择原图的时候不要使用原图上传，获取image时size稍微缩小一点这样可以保证上传快内存消耗小一点。在使用快速获取原图方法时,请将这个方法写在上传方法里! 在获取原图Image的过程中会比较消耗内存.
